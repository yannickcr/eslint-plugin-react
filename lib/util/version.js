...
function detectReactVersionFromContext(context) {
  try {
    const reactPath = resolve.sync('react', {basedir: context.getCwd()});
...
function getReactVersionFromContext(context) {
  ...
  if (context.settings && context.settings.react && context.settings.react.version) {
    ...
    if (settingsVersion === 'detect') {
      settingsVersion = detectReactVersionFromContext(context);
    }
