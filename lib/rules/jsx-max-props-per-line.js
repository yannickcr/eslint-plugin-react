  ...
  create(context) {
    ...
    return {
      JSXOpeningElement(node) {
        ...
        const maxPropsCount = linePartitionedProps.length <= 1
          ? maximumSingle
          : maximumMultiple;

        linePartitionedProps.forEach((propsInLine) => {
          // debugger
          if (propsInLine.length > maxPropsCount) {
            const name = getPropName(propsInLine[maxPropsCount]);

            context.report({
              node: propsInLine[maxPropsCount],
              message: `Prop \`${name}\` must be placed on a new line`,
              fix: generateFixFunction(propsInLine, maxPropsCount)
            });
          }
        });
      }
