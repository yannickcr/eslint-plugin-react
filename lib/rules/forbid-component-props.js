...
module.exports = {
  ...
  create(context) {
    ...
    function isAllowed(whitelist, tagName) {
      return whitelist.some(item => {
        if (item.startsWith('*')) {
          return tagName.endsWith(item.slice(1));
        } else if (item.endsWith('*')) {
          return tagName.startsWith(item.slice(0, -1));
        }
    ...
    function isForbidden(prop, tagName) {
      ...
      return typeof whitelist !== 'undefined' && (typeof tagName === 'undefined' || !isAllowed(whitelist, tagName));
    }
